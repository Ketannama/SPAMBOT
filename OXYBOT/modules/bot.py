...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        ...
        await jarvis.edit(f"HA BHAI BOL KISKO CHODNA H🫧\n» `{mp} ᴍꜱ`")
...
async def restart(e):
    if e.sender_id in SUDO_USERS:
        await e.reply(f"`𝙰𝚁𝙺 𝚇 𝚂𝙿𝙰𝙼🫧 THODA WAIT KR EK BETA ADD KRA H ABHI TIYAR HO RHA GAND MARNE KE LIYE.`")
        try:
            await X1.disconnect()
        except Exception:
            pass
    ...
    if event.sender_id == OWNER_ID:
        ...
        if str(target) in sudousers:
            ...
            if len(sudousers) > 0:
                ...
                newsudo = f"{target}"
            await ok.edit(f"» **ɴᴇᴡ ꜱᴜᴅᴏ ᴜꜱᴇʀ**: `{target}`\n» `SANTANI KE EK AUR BETA KAR DIYE HAI SUDO..BOT CHODNE KE LIYE READY HO RHA HAI`")
