...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"»THODA WAIT KR EK BETA ADD KRA H ABHI TIYAR HO RHA GAND MARNE KE LIYE")
        target = ""
        ...
        if str(target) in sudousers:
            await ok.edit(f"t(f"𝙳𝙰𝚁𝙺 𝚇 𝚂𝙿𝙰𝙼🫧 𝗦𝗔𝗡𝗔𝗧𝗔𝗡𝗜 𝗞𝗔 BETA !!")
        else:
            if len(sudousers) > 0:
                newsudo = f"{sudousers} {target}"
            else:
                newsudo = f"{target}"
            await ok.edit(f"» **ɴᴇᴡ ꜱᴜᴅᴏ ᴜꜱᴇʀ**THODA WAIT KR EK BETA ADD KRA H ABHI TIYAR HO RHA GAND MARNE KE LIYE.")
